...
pub fn init_db(data_dir: &str) -> DbPool {
    ...
    ensure_column(
        ...
        "files",
        "link_visibility",
        "ALTER TABLE files ADD COLUMN link_visibility TEXT NOT NULL DEFAULT 'public'",
    );
    ensure_column(
        &conn,
        "files",
    ...
    ensure_column(
        ...
        "folder_settings",
        "link_visibility",
        "ALTER TABLE folder_settings ADD COLUMN link_visibility TEXT NOT NULL DEFAULT 'public'",
    );
    ensure_column(
        &conn,
        "folder_settings",
        "updated_at",
        "ALTER TABLE folder_settings ADD COLUMN updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP",
    );
    ...
    ensure_column(
        &conn,
        "app_settings",
        "web_upload_chunk_size_mb",
        "ALTER TABLE app_settings ADD COLUMN web_upload_chunk_size_mb TEXT",
    );
    ensure_column(
        &conn,
        "app_settings",
        "web_upload_chunk_threshold_mb",
        "ALTER TABLE app_settings ADD COLUMN web_upload_chunk_threshold_mb TEXT",
    );
...
fn ensure_column(conn: &rusqlite::Connection, table: &str, column: &str, ddl: &str) {
    ...
    if !has_column {
        ...
        if let Err(e) = conn.execute(ddl, []) {
            tracing::error!(
                "Migration warning: Failed to add {}.{}: {}",
                table,
                column,
                e
            );
...
fn inherited_folder_visibility_from_conn(conn: &rusqlite::Connection, folder_path: &str) -> String {
...
pub fn add_file_metadata_with_storage_in_folder(
    ...
    let inherited_visibility =
        inherited_folder_visibility_from_conn(&conn, &normalized_folder_path);
    ...
    let files = stmt
        .query_map([], |row| {
            Ok(FileMetadata {
                ...
                storage_backend: row
                    .get::<_, String>(5)
                    .unwrap_or_else(|_| constants::STORAGE_BACKEND_TELEGRAM.to_string()),
...
/// Rename and/or move a single file to a new folder_path/filename in one
/// atomic UPDATE. Used by WebDAV MOVE. Does not touch the underlying
/// Telegram message - only the metadata row.
pub fn rename_file(
    pool: &DbPool,
    identifier: &str,
    new_folder_path: &str,
    ...
    let conn = pool.get()?;
    let normalized_folder = normalize_folder_path(new_folder_path);
    let inherited_visibility = inherited_folder_visibility_from_conn(&conn, &normalized_folder);
    let rows = conn.execute(
        "UPDATE files
         SET filename = ?1,
             folder_path = ?2,
             link_visibility = ?3,
             expires_at = NULL
         WHERE short_id = ?4 OR file_id = ?4",
        params![new_filename, normalized_folder, inherited_visibility, identifier],
    )?;
    Ok(rows > 0)
}

/// Move/rename a folder (and everything under it) to a new path in one
/// statement, by rewriting the `folder_path` prefix for every affected file
/// and folder_settings row. Used by WebDAV MOVE on collections.
pub fn rename_folder_path(
    ...
    let conn = pool.get()?;
    let old_normalized = normalize_folder_path(old_path);
    let new_normalized = normalize_folder_path(new_path);
    if old_normalized.is_empty() || new_normalized.is_empty() || old_normalized == new_normalized {
        return Ok(0);
    }

    let old_prefix_like = format!("{}/%", old_normalized);
    let suffix_start = old_normalized.len() as i64 + 1;

    let rows = conn.execute(
        "UPDATE files
         SET folder_path = ?1 || substr(folder_path, ?2)
         WHERE folder_path = ?3 OR folder_path LIKE ?4",
        params![new_normalized, suffix_start, old_normalized, old_prefix_like],
    )?;
...
pub fn create_album(
    ...
    let resolved_cover_file_id = match cover_file_id {
        Some(identifier) if !identifier.trim().is_empty() => {
            resolve_file_id(pool, identifier.trim())?
        }
        _ => None,
    };
    conn.execute(
...
pub fn get_app_settings_from_db(
    ...
    let conn = pool.get()?;
    let result = conn.query_row(
        "SELECT bot_token, channel_name, pass_word, picgo_api_key, base_url, session_token, storage_backend, s3_endpoint, s3_region, s3_bucket, s3_access_key, s3_secret_key, s3_public_base_url, webdav_enabled, webdav_readonly, webdav_username, web_upload_chunk_size_mb, web_upload_chunk_threshold_mb FROM app_settings WHERE id = 1",
        [],
        |row| {
            ...
            map.insert("WEB_UPLOAD_CHUNK_SIZE_MB".to_string(), row.get::<_, Option<String>>(16)?);
            map.insert("WEB_UPLOAD_CHUNK_THRESHOLD_MB".to_string(), row.get::<_, Option<String>>(17)?);
            Ok(map)
        },
    );
...
pub fn save_app_settings_to_db(
    ...
    let conn = pool.get()?;
    conn.execute(
        "UPDATE app_settings SET bot_token = ?1, channel_name = ?2, pass_word = ?3, picgo_api_key = ?4, base_url = ?5, session_token = ?6, storage_backend = ?7, s3_endpoint = ?8, s3_region = ?9, s3_bucket = ?10, s3_access_key = ?11, s3_secret_key = ?12, s3_public_base_url = ?13, webdav_enabled = ?14, webdav_readonly = ?15, webdav_username = ?16, web_upload_chunk_size_mb = ?17, web_upload_chunk_threshold_mb = ?18 WHERE id = 1",
        params![
            ...
            norm(payload.get("WEB_UPLOAD_CHUNK_SIZE_MB").and_then(|v| v.as_deref())),
            norm(payload.get("WEB_UPLOAD_CHUNK_THRESHOLD_MB").and_then(|v| v.as_deref())),
        ],
    )?;
    Ok(())
}
...
pub fn reset_app_settings_in_db(pool: &DbPool) -> Result<(), AppErrorKind> {
    ...
    payload.insert(
        ...
        Some(constants::STORAGE_BACKEND_TELEGRAM.to_string()),
    );
    ...
    payload.insert(
        "WEB_UPLOAD_CHUNK_SIZE_MB".to_string(),
        Some("64".to_string()),
    );
    payload.insert(
        "WEB_UPLOAD_CHUNK_THRESHOLD_MB".to_string(),
        Some("90".to_string()),
    );
