...
impl TelegramService {
    ...
    async fn send_document(
        ...
        let mime_type = mime_guess::from_path(filename)
            ...
            .to_string();
        // `Part::stream` (backed by `Bytes`) avoids the extra Bytes->Vec<u8>
        // copy that `Part::bytes` would require; the length is still known
        // up-front since `reqwest::Body::from(Bytes)` is a sized body, so
        // the multipart Content-Length is computed correctly either way.
        let content_length = file_bytes.len() as u64;
