    ...
    let (file_id, file_name, file_size) = if let Some(doc) = &message.document {
        ...
        (
            ...
            video
                .file_name
                .clone()
                .unwrap_or_else(|| format!("video_{}.mp4", message.message_id)),
...
async fn handle_get_reply(message: &Message, tg_service: &TelegramService, base_url: &str) {
    ...
    let (file_id, file_name) = if let Some(doc) = &replied.document {
        ...
        (
            ...
            video
                .file_name
                .clone()
                .unwrap_or_else(|| format!("video_{}.mp4", replied.message_id)),
        )
