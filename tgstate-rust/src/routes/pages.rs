...
fn share_relative_url(display_id: &str) -> String {
    format!("/d/{}", display_id)
}
    ...
    match meta {
        Ok(Some(f)) => {
            let display_id = f
                ...
                .filter(|s| !s.is_empty())
                .unwrap_or(&f.file_id);
            let relative_url = share_relative_url(display_id);
...
#[cfg(test)]
mod tests {
    use super::share_relative_url;

    #[test]
    fn share_page_uses_short_download_route() {
        assert_eq!(share_relative_url("abc123"), "/d/abc123");
    }
}
