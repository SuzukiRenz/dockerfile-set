...
fn validate_config(
    ...
    if let Some(Some(backend)) = cfg.get("STORAGE_BACKEND") {
        let b = backend.trim();
        if !b.is_empty() && b != "telegram" {
            return Err((
                axum::http::StatusCode::BAD_REQUEST,
                "STORAGE_BACKEND 仅支持 telegram",
    ...
    if let Some(Some(username)) = cfg.get("WEBDAV_USERNAME") {
        if username.trim().is_empty() {
            return Err((
                axum::http::StatusCode::BAD_REQUEST,
                "WEBDAV_USERNAME 不能为空",
                "invalid_webdav_username",
            ));
        }
    }
    if let Some(Some(chunk_size)) = cfg.get("WEB_UPLOAD_CHUNK_SIZE_MB") {
        let raw = chunk_size.trim();
        if !raw.is_empty() {
            let mb = match raw.parse::<u64>() {
                ...
                Err(_) => {
                    return Err((
                        axum::http::StatusCode::BAD_REQUEST,
                        "网页分片大小必须是数字",
                        "invalid_web_upload_chunk_size",
                    ));
                }
            };
            if !(5..=2048).contains(&mb) {
                return Err((
                    axum::http::StatusCode::BAD_REQUEST,
                    "网页分片大小需在 5-2048 MB 之间",
                    "invalid_web_upload_chunk_size",
                ));
            }
        }
            ...
            let mb = match raw.parse::<u64>() {
                ...
                Err(_) => {
                    return Err((
                        axum::http::StatusCode::BAD_REQUEST,
                        "分片触发阈值必须是数字",
                        "invalid_web_upload_chunk_threshold",
                    ));
                }
            };
...
> {
    ...
    if incoming.storage_backend.is_some() {
    ...
    set_opt!(
        incoming.web_upload_chunk_size_mb,
        "WEB_UPLOAD_CHUNK_SIZE_MB"
    );
    set_opt!(
        incoming.web_upload_chunk_threshold_mb,
...
fn copy_live_database_to_backup(
    state: &AppState,
...
async fn save_config_only(
    ...
    Ok(Json(
        serde_json::json!({"status": "ok", "message": "已保存（未应用）"}),
    ))
}
...
async fn import_database_backup(
    ...
    let temp_restore_path =
        std::path::Path::new(&state.settings.data_dir).join("file_metadata.restore.tmp.db");
...

    {
        ...
        source
            ...
            .map_err(|e| {
                crate::error::AppError::new(
                    axum::http::StatusCode::BAD_REQUEST,
                    &format!("备份数据库完整性检查失败: {}", e),
                    "db_restore_integrity_check_failed",
                )
            })
            .and_then(|value| {
...
async fn set_password(
    ...
    Ok(Json(
        serde_json::json!({"status": "ok", "message": "密码已成功设置。"}),
    ))
}
...
async fn verify_bot(
    ...
    if token.is_empty() {
        return Json(
            serde_json::json!({"status": "ok", "ok": false, "available": false, "message": "未提供 BOT_TOKEN"}),
        );
    }
        ...
        Ok(resp) => match resp.json::<serde_json::Value>().await {
            Ok(data) => {
                if data["ok"].as_bool() == Some(true) {
                    ...
                    Json(
                        serde_json::json!({"status": "ok", "ok": true, "available": true, "result": {"username": username}}),
                    )
                } else {
                    Json(
                        serde_json::json!({"status": "ok", "ok": false, "available": false, "message": data["description"].as_str().unwrap_or("Unknown error")}),
                    )
                }
            }
            Err(e) => {
                ...
                Json(
                    serde_json::json!({"status": "ok", "ok": false, "available": false, "message": "解析响应失败"}),
                )
            }
        },
        Err(e) => {
            ...
            Json(
                serde_json::json!({"status": "ok", "ok": false, "available": false, "message": "连接失败"}),
            )
        }
    }
}
...
async fn verify_channel(
    ...
    if token.is_empty() || channel.is_empty() {
        return Json(
            serde_json::json!({"status": "ok", "available": false, "message": "缺少 BOT_TOKEN 或 CHANNEL_NAME"}),
        );
    }
        ...
        Ok(resp) => match resp.json::<serde_json::Value>().await {
            Ok(data) => {
                if data["ok"].as_bool() == Some(true) {
                    ...
                    Json(
                        serde_json::json!({"status": "ok", "available": false, "message": data["description"].as_str().unwrap_or("Unknown error")}),
                    )
                }
            }
            Err(e) => {
                ...
                Json(
                    serde_json::json!({"status": "ok", "available": false, "message": "解析响应失败"}),
                )
            }
        },
        Err(e) => {
