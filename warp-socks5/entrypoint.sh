...
# IP 栈优先级：留空=系统默认(通常IPv4优先) | 6=IPv6优先 | 4=IPv4优先
PREFER_IP="${PREFER_IP:-}"
...
validate_args() {
    ...
    case "$PREFER_IP" in
        ""|4|6) ;;
        *) yellow "PREFER_IP 值 '${PREFER_IP}' 无效，忽略（可选：留空 | 4 | 6）"; PREFER_IP="" ;;
    esac
}
...
# ════════════════════════════════════════════════════════════════
#  IP 栈优先级设置（修改 /etc/gai.conf）
#  原理：gai.conf 控制 getaddrinfo() 返回地址的排序
#    IPv6 优先：添加 label 2002::/16 2（降低 IPv4-mapped 优先级）
#    IPv4 优先：添加 precedence ::ffff:0:0/96 100
#    默认恢复：删除上述两条，让系统自行决定
# ════════════════════════════════════════════════════════════════
apply_ip_priority() {
    # 先清理之前可能留下的规则
    if [[ -f /etc/gai.conf ]]; then
        sed -i '/^precedence ::ffff:0:0\/96/d;/^label 2002::\/16/d' /etc/gai.conf 2>/dev/null || true
    fi

    case "$PREFER_IP" in
        6)
            echo "label 2002::/16   2" >> /etc/gai.conf
            green "IP 优先级：IPv6 优先"
            ;;
        4)
            echo "precedence ::ffff:0:0/96  100" >> /etc/gai.conf
            green "IP 优先级：IPv4 优先"
            ;;
        *)
            green "IP 优先级：系统默认"
            ;;
...
print_info() {
    ...
    local PRIO_LABEL
    case "$PREFER_IP" in
        6) PRIO_LABEL="IPv6 优先" ;;
        4) PRIO_LABEL="IPv4 优先" ;;
        *) PRIO_LABEL="系统默认"  ;;
    esac
